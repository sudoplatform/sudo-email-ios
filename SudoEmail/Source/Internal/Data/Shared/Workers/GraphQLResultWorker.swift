...
struct GraphQLResultWorker {
    ...
    ///
    /// All errors are converted to a `SudoEmailError.internalError`. If unexpected values are input or bad state with no specific error, a default error will
    /// be returned. The default error is:
    /// > Invalid result returned from Email Service
    /// - Parameters:
    ...
    func invalidResult<D>(_ error: Error? = nil) -> Swift.Result<D, Error> {
        let errorDescription = error?.localizedDescription ?? "Invalid result returned from Email Service"
