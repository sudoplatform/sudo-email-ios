...
/// Gets the list of all email messages for the user.
class ListEmailMessagesUseCase {
    ...
    /// - Parameters:
    ///   - input: Parameters used to retrieve a list of all email messages.
    func execute(
        withInput input: ListEmailMessagesInput
    ) async throws -> ListAPIResult<EmailMessageEntity, PartialEmailMessageEntity> {
        let sealedEmailMessages = try await emailMessageRepository.listEmailMessages(
            withInput: input
        )
